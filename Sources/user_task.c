...
int main() {
    ...
    while(true){
        ...
        if (vis_sync_result.found) {
            ...
            position += 0.61 * SAMPLE_RATE; // 更新数据指针位置
            break; // 找到同步头后退出循环
        }
        search_start += 0.1; // 每次搜索1s,步长100ms
    }
    ...
    while (true)
    {
        ...
        if (line_sync_result.found) {
            ...
            position += (0.003 * SAMPLE_RATE); // 更新数据指针位置
            // 处理该行Y值
            ...
            if(parity_result.found) {
                ...
                position += (0.0015 * SAMPLE_RATE); // 更新数据指针位置
                if(parity_result.is_odd) {
                    // 奇数行
                    process_line_by_fx(file_i, file_q, position, fx_by_lines[line / 2]);
                    position += BY_NUM; // 更新数据指针位置
                } else {
                    // 偶数行
                    process_line_ry_fx(file_i, file_q, position, fx_ry_lines[line / 2]);
                    position += RY_NUM; // 更新数据指针位置
                }
            }
            line++;
            ...
            position += 0.15 * SAMPLE_RATE; // 更新数据指针位置
        }
    }
...
void process_line_y_fx(FILE* file_i, FILE* file_q, int start_sample, short* y_line_fx) {
    ...
    if(start_sample + Y_NUM > TOTAL_SAMPLES) {
        printf("警告：start_sample %d 的Y信号超出采样范围\n",start_sample);
        // 填充零值
...
void process_line_ry_fx(FILE* file_i, FILE* file_q, int ry_start, short* ry_line_fx) {
    ...
    if(ry_start + RY_NUM > TOTAL_SAMPLES) {
        printf("警告：ry_start %d 的RY信号超出采样范围\n",ry_start);
        for(int i = 0; i < IMAGE_WIDTH; i++) {
            ry_line_fx[i] = 0;
        }
    } else {
...
void process_line_by_fx(FILE* file_i, FILE* file_q, int by_start, short* by_line_fx) {
    ...
    if(by_start + BY_NUM > TOTAL_SAMPLES) {
        printf("警告：by_start %d 的BY信号超出采样范围\n",by_start);
        for(int i = 0; i < IMAGE_WIDTH; i++) {
            by_line_fx[i] = 0;
        }
    } else {
